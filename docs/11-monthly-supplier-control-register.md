...
| Ongoing | Digital Mobility Solutions Lanka / PickMe | Actual-usage prepayment | Ongoing | Pull current top-ups, active allocations and remaining supplier prepayments from Xero | Actual supported monthly usage | 430 | Do not create equal recurring bills. Match the reference and usage description to the service month; date the bill on the first of that month and use month-end as the due date. Consume the prepayment funding the current bill sequence before the next top-up, splitting a bill when needed. Keep unsupported older residuals separate. |
