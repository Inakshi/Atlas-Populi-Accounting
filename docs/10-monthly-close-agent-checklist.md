...
- [ ] **MC-43A — Validate the destination of every supplier credit.** Trace each active overpayment or credit allocation back to its payment voucher, bank debit, supplier account or connection, and service month. Confirm the destination bill represents the same obligation. A paid or nearly paid bill is not evidence that the allocation is correct. If a June payment was applied to a July bill, treat it as a `FAIL`: remove that allocation, apply the original credit to the June bill with the supported date, and leave the July bill open until separate exact evidence is found. Never create a second payment to conceal a misallocation.
